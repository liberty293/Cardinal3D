...
Spectrum Pathtracer::trace_pixel(size_t x, size_t y) {
    ...
    {
        Samplers::Rect::Uniform Sampler(Vec2(1));
        float pdf;
        Vec2 pt(Sampler.sample(pdf));
        xy += pt;
    }
...
Spectrum Pathtracer::trace_ray(const Ray& ray) {
    ...
    Spectrum radiance_out = Spectrum(0.0f);
    {
        ...
        auto sample_light = [&](const auto& light) {
            ...
            for(int i = 0; i < samples; i++) {
                ...
                shadow.dist_bounds = Vec2(EPS_F, sample.distance);
                Trace hit = scene.hit(shadow);
                if (hit.hit) continue;
    ...
    BSDF_Sample in_sample = bsdf.sample(out_dir);
    radiance_out += in_sample.emissive;
    ...
    Spectrum throughput;
    if (bsdf.is_discrete())
        throughput = in_sample.attenuation / in_sample.pdf;
    else
        throughput = in_sample.attenuation * in_sample.direction.y / in_sample.pdf;
    float q = std::min(std::max(1 - throughput.max_component(), 0.0f), 0.8f);
    if (RNG::unit() < q)
        return radiance_out;
